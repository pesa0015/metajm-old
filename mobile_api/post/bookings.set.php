...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (isset($_SESSION['company']['id']) && is_numeric($_SESSION['company']['id']) && isset($_SESSION['me']['id']) && is_numeric($_SESSION['me']['id'])){
		...
		if ($timeAlreadyBooked) {
			...
			if ($customer) {
				$end = $end->modify('+1 minute');
				if (sqlAction("INSERT INTO bookings (booked_at, start, end, invoice, webpay, in_place, company_id, employer_id, service_id, customer_id) VALUES (now(), '{$start->format('Y-m-d H:i:s')}', '{$end->format('Y-m-d H:i:s')}', 0, 0, 0, {$_SESSION['company']['id']}, {$_SESSION['me']['id']}, {$customer[0]['service_id']}, {$customer[0]['customer_id']});")) {
					echo 1;
					die;
				}
			}
